...
SEO FOUNDATIONS (`src/lib/seo.js`, `hooks/useDocumentMeta.js`, called once from App): per-route
title / description / canonical / OG / Twitter / JSON-LD (WebSite+Person on home, Person on
about, VideoObject or CreativeWork per project). Static defaults + share card (`public/og.jpg`,
1200×630) in index.html. `public/robots.txt`, `public/sitemap.xml` (26 URLs, regenerated by
`prebuild`), `dist/404.html` (= index, via `postbuild`). Verified live. **No `sameAs` and no
